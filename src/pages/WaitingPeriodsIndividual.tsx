...
import waitingPeriodsData from '../data/waitingPeriodsIndividual.json'

const waitingPeriods: WaitingPeriod[] = waitingPeriodsData

export default function WaitingPeriodsIndividual() {
  const [searchQuery, setSearchQuery] = useState('')

  const filteredPeriods = filterBySubstring(
    waitingPeriods,
    searchQuery,
    (item) => item.procedure
  )

  const accordionItems: AccordionItem[] = filteredPeriods.map((period) => ({
    id: period.id,
      ...
      <section className="bg-primary-900 py-16 tablet:py-20 px-4 tablet:px-8">
        <div className="mx-auto max-w-4xl">
          ...
          <p className="text-body-lg text-white/75 mt-4 max-w-2xl">
            Consulte os prazos de carencia para procedimentos do plano individual.
            Utilize o campo de busca para encontrar um procedimento especifico.
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-4xl">
          ...
          {accordionItems.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-2xl border border-warm-200">
              <p className="text-body text-warm-600">
                Nenhum procedimento encontrado. Tente ajustar o termo de busca.
              </p>
            </div>
          )}
        </div>
